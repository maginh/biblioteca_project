...
if($set){
  $up = $dbconn->query("UPDATE prestiti SET flag=1 WHERE numero=(SELECT numero FROM prestiti WHERE dataR=ADDDATE('{$row[0]}', INTERVAL -1 DAY) AND fNoDelete=1 AND LIBRO={$nLibro} AND flag=0 ORDER BY numero LIMIT 1)");
  if($up){
    echo("<script>alert('Prestito eliminato con successo');window.location.href='profilo.php';</script>");
  }else{
    echo("<script>alert('Non siamo riusciti ad eliminare il libro come richiesto, ci spiace per l'inconveniente');window.location.href='profilo.php';</script>");
  }
}else{
  echo("<script>alert('Non siamo riusciti ad eliminare il libro come richiesto, ci spiace per l'inconveniente');window.location.href='profilo.php';</script>");
}
